...
class EditorHeaderContainer extends Component {
  ...
  handleSubmit = async () => {
    ...
    const post = {
      title,
      body: markdown,
      // 태그 텍스트를 ,로 분리시키고 앞뒤 공백을 지운 후 중복 값 제거
    ...
    try {
      ...
      // 페이지를 이동, 주의: postId는 위쪽에서 레퍼런스를 만들지 않고
      // 이자리에서 this.props.postId를 조회해야 함 (현재값을 불러오기 위해)
...
)(withRouter(EditorHeaderContainer)); // for history.goBack()
